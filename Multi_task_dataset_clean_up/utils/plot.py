...
def plot_detect_sample_statistics(dataset) -> None:
    """[绘制样本统计图]
    ...
    x = np.arange(len(dataset['detect_class_list_new']))  # x为类别数量
    fig = plt.figure(1, figsize=(
        len(dataset['detect_class_list_new']), 9))   # 图片宽比例为类别个数
...
def plot_segment_sample_statistics(dataset) -> None:
    """[绘制样本统计图]
    ...
    if 'unlabeled' in dataset['segment_class_list_new']:
        x = np.arange(len(dataset['segment_class_list_new']))  # x为类别数量
    else:
        x = np.arange(len(dataset['segment_class_list_new']) + 1)  # x为类别数量
    fig = plt.figure(1, figsize=(
