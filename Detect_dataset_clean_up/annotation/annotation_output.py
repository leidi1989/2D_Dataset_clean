...
LastEditors: Leidi
LastEditTime: 2021-10-29 10:54:18
'''
import os
import json
...
from annotation.dataset_output_function import yolo, pascal_voc


def PASCAL_VOC_OUTPUT(dataset: dict) -> None:
...
def YOLO_OUTPUT(dataset: dict) -> None:
    """[输出temp dataset annotation]
    ...
    print('\nTemp dataset transform to YOLO:')
    pool = multiprocessing.Pool(dataset['workers'])
    for temp_annotation_path in tqdm(dataset['temp_annotation_path_list']):
        pool.apply_async(func=yolo.annotation_output,
                         args=(dataset, temp_annotation_path,),
                         error_callback=err_call_back)
    pool.close()
    pool.join()
...
annotation_out_function_dict = {'pascal_voc': PASCAL_VOC_OUTPUT,
                                'coco2017': COCO_2017_OUTPUT,
                                'yolo': YOLO_OUTPUT
                                }
