...
# def save_cut_bbox_image(cut_image_list, image_resize, output_image_path):
#     """[存储提取的bbox图片]
...
#         width = int(cut_box_image.shape[1] * image_resize)
#         height = int(cut_box_image.shape[0] * image_resize)
#         cut_box_image = cv2.resize(
#             cut_box_image, (width, height), cv2.INTER_CUBIC)
...
def creat_bbox_image(data_list, putout_image_style, output_image_path, pick_classes_list, image_expand, image_resize, save_image):
    """[创建源图片bbox剪切图片信息]
    ...
    for one_image in tqdm(data_list):     # 遍历全部annotation数据
        ...
        for i, one_bbox in enumerate(one_image.true_box_list):    # 遍历图片包含的bbox信息
            if one_bbox.clss not in pick_classes_list:      # 若bbox不在指定的提取类别列表内，则略过
                continue
            cut_box_image = img[
                int((one_bbox.ymin)*(1-image_expand)):int((one_bbox.ymax)*(1+image_expand)),
                # 读取bbox的ymin，ymax，xmin，xmax
                int((one_bbox.xmin)*(1-image_expand)):int((one_bbox.xmax)*(1+image_expand))]
            ...
            if save_image == 1:
                one_cut_image = cut_image(src_image_size,
                                          cut_box_image_name,
                ...
                cut_box_image = cv2.resize(
                    cut_box_image, (width, height), cv2.INTER_CUBIC)
                cv2.imwrite(cut_image_output_path,
                            cut_box_image)     # 将剪切的图片保存
            n += 1
    return n
