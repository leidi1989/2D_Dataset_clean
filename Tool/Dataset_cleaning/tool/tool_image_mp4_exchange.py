...
LastEditors: Leidi
LastEditTime: 2021-09-23 16:23:56
'''
# 导入需要的库
import cv2
import os
...
import sys
sys.path.append('/home/leidi/hy/hy_program/2D_Dataset_clean/Dataset_cleaning')
...
def img2mp4(image_folder_path: str, mp4fps: int) -> None:
    ...
    fourcc = cv2.VideoWriter_fourcc(*'MPEG')   # 视频写入编码器
    videoWrite = cv2.VideoWriter(mp4_file_path, fourcc, mp4fps, size)
    # 根据图片的大小，创建写入对象(文件名，支持的编码器，帧频，视频大小(图片大小))
    for _, name in tqdm(enumerate(image_list)):
        fileName = image_folder_path + '/' + name  # 读取所有图片的路径
...
if __name__ == "__main__":
    ...
    parser.add_argument('--imgpath', default=r'/home/leidi/Dataset/detect_output/YOLOP/20210923/cam_left_front_20210918_output_20210918',
                        type=str, help='image output path')
    parser.add_argument('--pref', default=r'',
                        type=str, help='rename prefix')
    parser.add_argument('--mode', default='image',
                        type=int, help='image output')
