...
LastEditors: Leidi
LastEditTime: 2021-12-31 14:30:49
'''
import os
import cv2
import json
...
from utils.convertion_function import coco_voc
    ...
    for target_annotation in dataset['check_file_name_list']:
        ...
        for one_annotation in tqdm(data['annotations']):
            if one_annotation['image_id'] in images_data_dict:
                ...
                if cls not in dataset['class_list_new']:
                    continue
                bbox = coco_voc(one_annotation['bbox'])
                ...
                images_data_dict[ann_image_id].true_box_list_updata(
                    TRUE_BOX(cls, bbox[0], bbox[2], bbox[1], bbox[3]))
