...
LastEditors: Leidi
LastEditTime: 2022-07-19 11:09:08
'''
import time
import shutil
...
class CVAT_COCO2017(Dataset_Base):
    ...
    def transform_to_temp_dataset(self) -> None:
        ...
        for source_annotation_name in tqdm(os.listdir(
            ...
            for id, one_annotation in enumerate(data['annotations']):
                total_image_annotation_list.append(
                    pool.apply_async(func=self.load_image_annotation,
                                     args=(id, one_annotation, class_dict,
                                           total_annotations_dict),
                                     callback=update,
                                     error_callback=err_call_back))
            pool.close()
            ...
            for image_true_annotation in total_image_annotation_list:
                if image_true_annotation.get()[1] is None:
                    continue
                if image_true_annotation.get(
                )[0] not in total_images_data_dict:
                    total_images_data_dict[image_true_annotation.get()
                                           [0]] = total_annotations_dict[
                                               image_true_annotation.get()[0]]
                    total_images_data_dict[image_true_annotation.get()
                                           [0]].object_list.append(
    ...
    def load_image_base_information(self, image_base_information: dict,
                                    total_annotations_dict: dict) -> None:
    ...
    def load_image_annotation(self, id: int, one_annotation: dict,
                              class_dict: dict,
                              each_annotation_images_data_dict: dict) -> list:
        ...
        if 'bbox' in one_annotation and len(one_annotation['bbox']):
            box = [
                one_annotation['bbox'][0], one_annotation['bbox'][1],
                one_annotation['bbox'][0] + one_annotation['bbox'][2],
                one_annotation['bbox'][1] + one_annotation['bbox'][3]
            ]
            xmin = max(min(int(box[0]), int(box[2]), int(image.width)), 0.)
            ymin = max(min(int(box[1]), int(box[3]), int(image.height)), 0.)
            xmax = min(max(int(box[2]), int(box[0]), 0.), int(image.width))
            ymax = min(max(int(box[3]), int(box[1]), 0.), int(image.height))
            box_xywh = [xmin, ymin, xmax - xmin, ymax - ymin]

        # 获取真实语义分割信息
        if 'segmentation' in one_annotation and len(
            ...
            for one_seg in one_annotation['segmentation']:
                ...
                for i, x in enumerate(one_seg):
                    if 0 == i % 2:
                        ...
                        if 2 != len(point):
                            print(
                                'Segmentation label wrong: ',
                                each_annotation_images_data_dict[ann_image_id].
                                image_name_new)
                            continue
                        point = []
    ...
    def output_temp_annotation(self, image: IMAGE,
    ...
    def target_dataset(dataset_instance: Dataset_Base) -> None:
        ...
        for dataset_temp_annotation_path_list in tqdm(
            ...
            for task, task_class_dict in tqdm(
                ...
                pool = multiprocessing.Pool(dataset_instance.workers)
                for n, temp_annotation_path in tqdm(
                        enumerate(annotation_path_list)):
    ...
    @staticmethod
    def get_image_information(dataset_instance: Dataset_Base, coco: dict,
                              n: int, temp_annotation_path: str) -> dict:
        """读取暂存annotation
        ...
        # 图片基础信息
        image_information = {
            'license': random.randint(0, len(coco['licenses'])),
    ...
    @staticmethod
    def get_annotation(dataset_instance: Dataset_Base, n: int,
        ...
        for object in image.object_list:
            ...
            if task == 'Detection':
                ...
                if clss not in task_class_dict['Target_dataset_class']:
                    continue
                category_id = dataset_instance.temp_merge_class_list[
                    'Merge_target_dataset_class_list'].index(clss)
                one_image_annotations_list.append({
                    'bbox': object.box_xywh,
                ...
                one_image_annotations_list.append({
                    ...
                    n,
                    'category_id':
                    dataset_instance.temp_merge_class_list[
                        'Merge_target_dataset_class_list'].index(clss),
                    'id':
                    0
                })
    ...
    def annotation_check(dataset_instance: Dataset_Base) -> list:
        ...
        for target_annotation in tqdm(
                dataset_instance.target_check_file_name_list,
            ...
            total_image_count = 0
            for d in tqdm(data['images'], desc='Count images', leave=False):
                total_image_count += 1
            check_images_count = min(
                dataset_instance.target_dataset_annotations_check_count,
                total_image_count)
            check_image_id_list = [
                random.randint(0, total_image_count)
                for i in range(check_images_count)
            ]
            ...
            for n in tqdm(check_image_id_list,
                ...
                img_path = os.path.join(dataset_instance.temp_images_folder,
                                        img_name_new)
                ...
                # 将获取的图片名称、图片路径、高、宽作为初始化per_image对象参数，
                # 并将初始化后的对象存入total_images_data_list
                image = IMAGE(img_name, img_name_new, img_path, height, width,
                              channels, [])
            ...
            for one_annotation in tqdm(data['annotations'],
                ...
                if one_annotation['image_id'] in images_data_dict:
                    ...
                    # 获取真实框信息
                    if 'bbox' in one_annotation and len(
                            one_annotation['bbox']):
                        box = [
                            ...
                            one_annotation['bbox'][1] +
                            one_annotation['bbox'][3]
                        ]
                        xmin = max(
                            min(int(box[0]), int(box[2]), int(image.width)),
                            0.)
                        ymin = max(
                            min(int(box[1]), int(box[3]), int(image.height)),
                            0.)
