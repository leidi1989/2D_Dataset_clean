...
LastEditors: Leidi
LastEditTime: 2022-07-19 11:12:47
'''
...
class BDD100K(Dataset_Base):
    ...
    def load_image_annotation(self, source_annotation_name: str,
        ...
        area_list = ['area/drivable', 'area/alternative', 'area/unknown']
        start_point_dist_threshhold = {
            'lane/crosswalk': 800,
        ...
        image_name = os.path.splitext(
            source_annotation_name)[0] + '.' + self.temp_image_form
        ...
        for a, object in enumerate(object_box_list):
            clss = object['category']
            clss = clss.replace(' ', '').lower()
            box_xywh = temp_box_to_coco_box([
                object['box2d']['x1'], object['box2d']['y1'],
                object['box2d']['x2'], object['box2d']['y2']
            ])
            object_list.append(
                OBJECT(a,
                       clss,
                ...
                if point[2] == 'L':
                    ...
                    if 3 == c_count:
                        ...
                        for r in range(1, 21):
                            r = r / 20
                            bezier_line.append(
                                calNextPoints(temp_point, rate=r)[0])
        ...
        for lane in object_segment_lane_list:
            ...
            if 2 == len(lane['poly2d']):
                ...
                lane.update({
                    'zero_to_start_point_dist':
                    dist(np.array(lane['line_point_list'][0]), np.array([0,
                                                                         0]))
                })
            else:
                segmentation_point_list = [x[0:-1] for x in lane['poly2d']]
                ...
                for r in range(1, 21):
                    r = r / 20
                    line_point_list.append(
                        calNextPoints(segmentation_point_list, rate=r)[0])
                line_point_list.sort(
                    key=lambda line_point: (line_point[1], -line_point[0]))
                lane.update({'line_point_list': line_point_list})
                lane.update({
                    'zero_to_start_point_dist':
                    dist(np.array(lane['line_point_list'][0]), np.array([0,
            ...
            for line in value:
                if not temp_line:
                    ...
                    if temp_line['category'] != key:
                        ...
                        line_points_dist = []
                        for m, n in zip(temp_line['line_point_list'],
                                        line['line_point_list']):
                        ...
                        lines_start_point_dist = dist(
                            np.array(temp_line['line_point_list'][0]),
                            np.array(line['line_point_list'][0]))
                        lines_end_point_dist = dist(
                            np.array(temp_line['line_point_list'][-1]),
                            np.array(line['line_point_list'][-1]))
        ...
        for c, [m, n] in enumerate(object_segment_double_line_lane_pair_list):
            ...
            for r in range(1, 21):
                r = r / 20
                line_point_list_2.append(
                    calNextPoints(segmentation_point_list, rate=r)[0])

            pair_line_dist_0_0 = dist(np.array(line_point_list_1[0]),
                                      np.array(line_point_list_2[0]))
            pair_line_dist_0_1 = dist(np.array(line_point_list_1[0]),
                                      np.array(line_point_list_2[-1]))
        ...
        for d, object in enumerate(object_segment_one_line_lane_list):
            ...
            if 2 == len(segmentation_point_list):
                line_point_list_1 = [[
                    x - one_line_expand_offset
                    for x in object['poly2d'][0][0:-1]
                ],
                                     [
                                         x - one_line_expand_offset
                                         for x in object['poly2d'][1][0:-1]
                                     ]]
                ...
                line_point_list_loop = np.append(line_point_list_l,
                                                 line_point_list_r,
                                                 axis=0)
            ...
            # 贝塞尔曲线
                ...
                for points in object['poly2d']:
                    line_point_list_1_c.append([
                        points[0:-1][0] - one_line_expand_offset,
                        points[0:-1][1]
                    ])
                for r in range(1, 21):
                    r = r / 20
                    line_point_list_1.append(
                        calNextPoints(line_point_list_1_c, rate=r)[0])
                # 单线右侧边缘
                line_point_list_2 = [[
                    x - one_line_expand_offset
                    for x in object['poly2d'][0][0:-1]
                ...
                for r in range(1, 21):
                    r = r / 20
                    line_point_list_2.append(
                        calNextPoints(line_point_list_2_c, rate=r)[0])
                ...
                line_point_list_loop = line_point_list_loop_temp
                object_list.append(
                    OBJECT(object_id=d + object_count,
                           object_clss=clss,
                           segmentation_clss=clss,
                           segmentation=line_point_list_loop,
                           need_convert=self.need_convert))
