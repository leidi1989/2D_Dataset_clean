...
LastEditors: Leidi
LastEditTime: 2022-07-19 11:10:12
'''
import time
import shutil
...
class GEELY_COCO_ONE_IMAGE(Dataset_Base):
    ...
    def load_image_annotation(self, source_annotation_name: str,
        ...
        image_name = os.path.splitext(
            data['images'][0]['file_name'])[0] + '.' + self.temp_image_form
        ...
        for one_annotation in data['annotations']:
            ...
            if 'bbox' in one_annotation and len(one_annotation['bbox']):
                ...
                xmin = max(min(int(box[0]), int(box[2]), int(width)), 0.)
                ymin = max(min(int(box[1]), int(box[3]), int(height)), 0.)
                xmax = min(max(int(box[2]), int(box[0]), 0.), int(width))
                ymax = min(max(int(box[3]), int(box[1]), 0.), int(height))
