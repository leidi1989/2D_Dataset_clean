...
LastEditors: Leidi
LastEditTime: 2022-08-12 18:43:23
'''
...
class TEMP_DATASET(Dataset_Base):
    ...
    def load_image_annotation(self, source_annotation_name: str,
        ...
        image = self.TEMP_LOAD(self, source_annotations_path)
        image_name = image.image_name_new
        image_name_new = self.file_prefix + image_name
        image_path = os.path.join(self.temp_images_folder, image_name_new)
        
        image.file_name = os.path.splitext(image_name)[0]
        image.file_name_new = os.path.splitext(image_name_new)[0]
        image.image_name = image_name
        image.image_name_new = image_name_new
